...
window.onload = function () {
    ...
    board(dom_course, dom_center_course, 140, 4, 'hello!');
    setInterval(draw, 50);  // この行を修正しました
}

// ボタンの部分の関数
function draw() {
    move_now = move_now + 0.05;
    board(dom_course, dom_center_course, 140, 4, 'hello!');
}
...
function board(canvas, canvas_center, size, angle, labal){
    ...
    for (let number = 0;  number < arc_number; number++){
        ...
        console.log(arc_number);
        //大きな円を書く
        draw_canvas.beginPath();
        draw_canvas.arc(canvas.width/2,canvas.height/2, size, ((Math.PI/180)*arc_angle)*number+(Math.PI/4) + move_now,  ((Math.PI/180)*arc_angle)*(number + 1)+(Math.PI/4) + move_now, false);//扇形のパス(複雑なのは円を上に持っていくため)
        ...
        draw_canvas.rotate(((Math.PI/180)*arc_angle )*number + move_now);
        draw_canvas.fillStyle = '#757575';//文字の設定(色)
        draw_canvas.font = '15px Arial';//文字の設定(大きさとフォント)
        draw_canvas.textAlign = 'center';//文字の設定(中央揃え)
        draw_canvas.fillText(labal,  0, -100);//外枠の文字を書く
        draw_canvas.restore();//座標系のセーブ復活
        draw_canvas.save(); //座標系セーブ
    }
    circle(canvas_center,size, labal);
}


function circle(canvas_center, size, labal){
    ...
    draw_canvas.fillText(labal, canvas_center.width/2, canvas_center.height/2);//ここで真ん中の数字を書いてる
}
